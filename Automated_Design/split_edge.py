...
def split_edge(edge_type_mat, num_vert):
    ...
    pseudo_vert = edge_type_mat_wHalfs.nodes() # Start with known nodes.  Pseudo nodes added to graph as needed.

    non_tree_edges = [edge for edge in edge_type_mat.edges(data=True) if edge[2]['type'] == 1]

    # ID non-tree edges (value 1), replace with 2 half-edges (value = -1)
    for edge in non_tree_edges:
        i, j, properties = edge  # `i` is the node that stays. `j` is cut off and replaced by pseudo vert.
                                 # Since this digraph was built from a graph, a link from 2,3 will also have
                                 # a link from 3,2.  Meaning in the 2,3 case, you'll keep 2 and replace 3.  And
                                 # in teh 3,2 case, you'll keep 3 and replace 2.
        ...
        edge_type_mat_wHalfs.add_edge(i, index_of_new_vert, attr_dict=properties)
        edge_type_mat_wHalfs.add_edge(index_of_new_vert, i, attr_dict=properties)
