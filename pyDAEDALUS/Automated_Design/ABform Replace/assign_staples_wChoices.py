...
Output: staples = cell array with E rows, each cell contains row vector.
    ...
    for edge_ID in range(num_edges):
        ...
        scaf_top_cut = scaf_top[11:-11]
        scaf_bot_cut = scaf_bot[11:-11]
        ...
        if len_cut > 0:  # if there are any other staples to be added
            if edge_type == 2:  # tree edge, no scaffold crossover
                # number of 22x2-nt staples
                num_22staps = int(floor(len_cut/22))
                # remaining staple, 11x2
                ...
                if len_cut == 22*num_22staps:
                    cutoffTop = int((len_cut/2.0)-8.0)
                    cutoffBot = int((len_cut/2.0)-3.0)
                else:  # scaffold crossover 0/1 away from center
                    cutoffTop = int((len_cut/2.0)-2.5)
                    cutoffBot = int((len_cut/2.0)+2.5)
    ...
    # # Join 11 and 11 staples with polyT to make 11+5+11 = 27 nt fragments
